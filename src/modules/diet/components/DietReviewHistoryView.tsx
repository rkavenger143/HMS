...
export default function DietReviewHistoryView() {
  const {
    admissions,
    dietCharts,
  ...
  const [modifyAdmId, setModifyAdmId] = useState<string | null>(null);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Diet Review & Historical Revision Log</div>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              Current diet plans, previous versions, modification reasons, clinical dietitian signoffs, and audit history
            </div>
          </div>
        </div>
        ...
        <button
          ...
          onClick={() => setModifyAdmId(activeAdmissions[0]?.id || null)}
        >
          <Edit size={13} /> Review & Modify Diet Plan
        </button>
      </div>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th>Reason for Change / Review Note</th>
                  <th>Dietitian / Modified By</th>
                  <th>Modified Date</th>
                  <th>Status</th>
              ...
              <tbody>
                {filteredCharts.map(ch => (
                  <tr key={ch.id}>
                    ...
                    {/* Modified By */}
                    <td>
                      <div style={{ fontSize: 12 }}>{ch.dietitianName}</div>
                    </td>
                      ...
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center' }}>
                        ...
                        {ch.status === 'active' && (
                          <button
                            className="btn btn-primary btn-sm"
                            style={{ fontSize: 11 }}
                            onClick={() => setModifyAdmId(ch.admissionId)}
                          >
                            <Edit size={12} /> Modify
                          </button>
                        )}
      ...
      {modifyAdmId && (
        <CreateDietChartModal
          initialAdmissionId={modifyAdmId}
          onClose={() => setModifyAdmId(null)}
        />
      )}
    </div>
  );
