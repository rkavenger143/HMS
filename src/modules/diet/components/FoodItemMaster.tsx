...
import { Apple, Plus, Search, Filter, Edit, CheckCircle2, Ban } from 'lucide-react';
import { useDiet } from '../context/DietContext';
import type { FoodItem } from '../../../types';

export default function FoodItemMaster() {
  ...
  const handleOpenEdit = (item: FoodItem) => {
    ...
    setPreparationMethod(item.preparationMethod || '');
    setShowAddModal(true);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Hospital Food Items & Nutrition Master Database</div>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              Standard portioning, nutritional macro values, allergen tagging, and preparation recipes
            </div>
          </div>
        </div>
      ...
      <div className="card" style={{ padding: '14px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, alignItems: 'center' }}>
          ...
          <select className="form-select" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
            ...
            <option value="supplements">Supplements & RTD</option>
                  ...
                  <tr key={item.id} style={{ opacity: item.isActive ? 1 : 0.6 }}>
                    <td>
                      ...
                      {item.preparationMethod && (
                        <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{item.preparationMethod}</div>
                      )}
                    </td>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{item.servingUnit}</div>
                    </td>
                    ...
                    <td>
                      <div style={{ fontSize: 12 }}>P: {item.protein}g · C: {item.carbs}g · F: {item.fat}g</div>
                    </td>

                    <td>
                      <span className={`badge ${item.isVegetarian ? 'badge-success' : 'badge-warning'}`}>
                      ...
                      {item.allergens.length > 0 ? (
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          {item.allergens.map((al, idx) => (
                            <span key={idx} className="badge badge-danger" style={{ fontSize: 10 }}>⚠️ {al}</span>
                          ))}
                        </div>
                      ) : (
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        <button className="btn btn-ghost btn-icon btn-icon-sm" onClick={() => handleOpenEdit(item)} title="Edit Item">
                          <Edit size={13} />
                        </button>
                        <button
                        ...
                        >
                          <Ban size={13} style={{ color: item.isActive ? 'var(--color-warning)' : 'var(--color-success)' }} />
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="modal-backdrop" onClick={() => setShowAddModal(false)}>
          <div className="modal modal-md" onClick={e => e.stopPropagation()} style={{ maxWidth: 540 }}>
            <div className="modal-header">
              ...
              <div className="modal-title">{editingItem ? 'Edit Food Item Master' : 'Add Food Item to Master'}</div>
              ...
              <div className="modal-body">
                <div className="form-grid form-grid-2" style={{ gap: 14 }}>
                  <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                    <label className="form-label">Food Item Name <span className="required">*</span></label>
                  ...
                  <div className="form-group">
                    ...
                    <select className="form-select" value={servingUnit} onChange={e => setServingUnit(e.target.value as any)}>
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" className="form-input" value={standardPortion} onChange={e => setStandardPortion(e.target.value)} />
                  </div>

                  <div className="form-group">
                    <label className="form-label">Calories (kcal)</label>
                    <input type="number" className="form-input" value={calories} onChange={e => setCalories(Number(e.target.value))} />
                  </div>

                  <div className="form-group">
                    <label className="form-label">Protein (g)</label>
                    <input type="number" className="form-input" value={protein} onChange={e => setProtein(Number(e.target.value))} />
                  </div>

                  <div className="form-group">
                    <label className="form-label">Carbs (g)</label>
                    <input type="number" className="form-input" value={carbs} onChange={e => setCarbs(Number(e.target.value))} />
                  </div>

                  <div className="form-group">
                    <label className="form-label">Fat (g)</label>
                    <input type="number" className="form-input" value={fat} onChange={e => setFat(Number(e.target.value))} />
                  </div>

                  <div className="form-group">
                    <label className="form-label">Type</label>
                    <select className="form-select" value={isVegetarian ? 'veg' : 'nonveg'} onChange={e => setIsVegetarian(e.target.value === 'veg')}>
                  ...
                  <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                    <label className="form-label">Allergens Present (Comma separated e.g. Gluten, Milk, Peanut)</label>
                    <input type="text" className="form-input" value={allergens} onChange={e => setAllergens(e.target.value)} />
                  </div>

                  <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                    <label className="form-label">Preparation Method / Kitchen Notes</label>
                    <input type="text" className="form-input" value={preparationMethod} onChange={e => setPreparationMethod(e.target.value)} />
                  </div>
                </div>
              </div>
